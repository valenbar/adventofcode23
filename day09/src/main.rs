...
fn main() -> Result<()> {
    ...
    let task_2 = solve_task_2("input.txt")?;
...
fn find_prev_value(vec: Vec<isize>) -> isize {
    let mut rows: Vec<Vec<isize>> = Vec::new();
    rows.push(vec.clone());
    for i in 0..vec.len() {
        let mut row: Vec<isize> = Vec::new();
    ...
    for i in (1..rows.len()).rev() {
        let diff: isize = *rows[i].first().unwrap();
        let first: isize = *rows[i-1].first().unwrap();
        rows[i-1].insert(0, first - diff);
    }
